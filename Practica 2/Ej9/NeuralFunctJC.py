...
###########################################################################
def graf4c(ejex,zm,zsg,titulo,ylabel):
    """
    ...
    ax1.set_ylabel(ylabel) # Configuramos la etiqueta del eje Y
    ax2.set_ylabel(ylabel) # Configuramos la etiqueta del eje Y
    ax3.set_ylabel(ylabel) # Configuramos la etiqueta del eje Y
    ax4.set_ylabel(ylabel) # Configuramos la etiqueta del eje Y
    #show
...
def nMk(k,r,t):
    ...
    return nMk
###########################################
def responsividad(tasasdisparo,tbasal,ventana):
    probabilidad=[] #lista vacía para los arrays de probabilidades de cada neurona
    for tasa in range(len(tasasdisparo)): # ciclo para cada neurona
        espigas=(tasasdisparo[tasa]*ventana).astype(int) #convertimos la tasa en frecuencia de neuronas
        r=tbasal[tasa]#tasa basal para esa neurona
        t=ventana# en este caso t que corresponde al tamaño de ventana es constante para todas las neuronas 
        prob=lambda espiga : nMk(espiga,r,t)# función anónima para calcular la probabilidad con los parametros especificos r t y k
##################################
        probas=[]# lista vacía con para las probabilidades de una neurona
        for i in range(len(espigas)): # este ciclo itera en el número de ensayos por neurona
            p=[prob(xi) for xi in espigas[i]] # realizamo las operación para obtener una probabilidad para cada elemento de cada ensayo de cada neurona 
            probas.append(p) # añadimos la lista de probabilidades para cada ventana de un ensayo a la lista probas
        probas=np.array(probas)# array de las probabilidades de una neurona
        probabilidad.append(probas)# agregamos el array a la lista de arrays
    return probabilidad #lista de arrays donde cada array corresponde a las probabilidades de los ensayos de una neurona
    
###################################
def G4clases(ejex,array,titulo,labely):
    """
    Esta función unicamente para listas con 4 clases.
    Dado un ejex comúm y un lista o tupla que contiene arrays,así como un título para la figura. Esta función crea una figura con las graficas del array promedio por ventan
    de cada clase. 
    """
    c1,c2,c3,c4=array# desempaquetamos los z score promedio
    
    # una figura
    fig,(ax1,ax2,ax3,ax4)=plt.subplots(nrows=4,ncols=1,sharex=True)
    # titulo
    fontdict_title = {'family': 'serif','color':  'darkred','weight': 'normal','size': 16,} # formato para el título
    ax1.set_title(titulo, fontdict_title) # creamos la gráfica 
    #######
    ax1.plot(ejex,c1,label='clase1',color='red')# creamos la gráfica 
    ax2.plot(ejex,c2,label='clase2',color='purple')# creamos la gráfica 
    ax3.plot(ejex,c3,label='clase3',color='darkblue')# creamos la gráfica 
    ...
    # labels para los ejes
    ax4.set_xlabel(" Time [s] ") # Configuramos la etiqueta del eje X
    ax1.set_ylabel(labely ) # Configuramos la etiqueta del eje Y
    ax2.set_ylabel( labely  ) # Configuramos la etiqueta del eje Y
    ax3.set_ylabel( labely  ) # Configuramos la etiqueta del eje Y
    ax4.set_ylabel( labely  ) # Configuramos la etiqueta del eje Y
    #show
    return plt.tight_layout()
############################################
def G6clases(ejex,array,titulo,ejey):
    """
    Esta función unicamente para listas con 6 clases.
    Dado un ejex comúm y un lista o tupla que contiene arrays ,
    así como un título para la figura. Esta función crea una figura con las 
    graficas del array promedio de cada clase. 
    """
    c1,c2,c3,c4,c5,c6=array
    # una figura
    fig,(ax1,ax2,ax3,ax4,ax5,ax6)=plt.subplots(nrows=6,ncols=1,sharex=True)
    # titulo
    fontdict_title = {'family': 'serif','color':  'darkred','weight': 'normal','size': 16,} # formato para el título
    ...
    ax3.plot(ejex,c3,label='clase3',color='darkblue')# creamos la gráfica 
    ax4.plot(ejex,c4,label='clase4',color='black')# creamos la gráfica 
    ax5.plot(ejex,c5,label='clase5',color='black')# creamos la gráfica 
    ax6.plot(ejex,c6,label='clase6',color='black')# creamos la gráfica 
    fig.legend()
    # labels para los ejes
    ax6.set_xlabel(" Time [s] ") # Configuramos la etiqueta del eje X
    ax1.set_ylabel(ejey) # Configuramos la etiqueta del eje Y
